...
window.addEventListener('error', function(e){
  try {
    ...
    if(!bar){
      ...
      bar.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;background:#f87171;color:#fff;padding:8px 16px;font-size:13px;cursor:pointer;';
      bar.onclick = function(){ this.style.display='none'; };
      document.body.appendChild(bar);
    }
    bar.style.display = 'block';
    bar.textContent = 'JS错误: ' + e.message + ' (' + (e.filename||'').split('/').pop() + ':' + e.lineno + ') 点击关闭';
...
// 全局视频共享状态：任一入口上传成功后写入，其他入口进入时自动复用（不再问用户再传一次）
let CURRENT_VIDEO = null;   // {file, name, url}
function setGlobalVideo(file){
  if(!file || !file.type.startsWith('video/')) return;
  CURRENT_VIDEO = { file, name: file.name, url: URL.createObjectURL(file) };
}

// 主上传区事件绑定（try-catch防止元素不存在时中断整个脚本）
try {
...
} catch(e) { console.warn('主上传区初始化失败:', e); }
...
function loadWhisperStatus(){
  fetch('/api/whisper/status').then(r=>r.json()).then(res=>{
    ...
    // [B] 展示推理设备；CPU 时给出启用 GPU 提示（P1-6）
    if(res.device){
      if(res.device === 'cuda') s += ' · 推理设备：🎮 GPU（' + (res.compute_type || 'float16') + ' 加速）';
      else s += ' · 推理设备：💻 CPU（' + (res.compute_type || 'int8') + '，建议启用 NVIDIA GPU，ASR 可提速约 5 倍）';
    }
...
const WHISPER_MODEL_CATALOG = [
  ...
  {tag:'large-v3-turbo',  label:'large-v3-turbo', desc:'最准·最快·GPU推荐（ASR 从30min→6min）', size:'≈1.5GB'},
];
...
function handle(file){
  ...
  ITEMS.push(it);
  if (it.kind === 'video') setGlobalVideo(file);  // 同步到全局共享
  render();
}
...
function gStart(label){
  ...
  $('gprogPct').textContent = '0.0%';
  $('gprogPhase').textContent = '';
  const metaEl = $('gprogMeta'); if (metaEl) metaEl.textContent = '⏱ 0s';
  $('gprogFill').style.width = '0.5%';
  $('gprog').classList.add('show');
}
function gSet(pct, phase, fails){
  const gc = $('gprogCancel'); if (gc) gc.style.display = _currentRunid ? '' : 'none';
  pct = Math.min(99.9, Math.max(0.1, pct || 0));
  $('gprogFill').style.width = pct + '%';
  $('gprogPct').textContent = pct.toFixed(1) + '%';
  const el = (Date.now() - _gStart) / 1000;
  // 已用时间格式化
  const elStr = el >= 60 ? Math.floor(el/60) + 'm' + Math.floor(el%60) + 's' : Math.floor(el) + 's';
  let metaStr = '⏱ ' + elStr;
  let txt = phase || '';
  // [2.2] 配音失败段对前端可见：进度条上即时展示「N 段配音失败，引擎已自动兜底」
  if (Array.isArray(fails) && fails.length){
    txt += '（⚠ ' + fails.length + ' 段配音失败，引擎已自动切换兜底，详情见任务记录）';
  }
  if (pct > 3 && el > 5){
    ...
    const etaStr = eta >= 60 ? Math.floor(eta/60) + 'm' + Math.floor(eta%60) + 's' : Math.ceil(eta) + 's';
    metaStr += ' · 剩余 ' + etaStr;
    // 处理速度（百分比/秒）
    const speed = pct / el;
    if (speed > 0) metaStr += ' · ' + speed.toFixed(2) + '%/s';
  }
  const metaEl = $('gprogMeta'); if (metaEl) metaEl.textContent = metaStr;
  $('gprogPhase').textContent = txt;
}
function gDone(){
  $('gprogFill').style.width = '100%';
  $('gprogPct').textContent = '100.0%';
  $('gprogPhase').textContent = '完成 ✓';
  const el = (Date.now() - _gStart) / 1000;
  const elStr = el >= 60 ? Math.floor(el/60) + 'm' + Math.floor(el%60) + 's' : Math.floor(el) + 's';
  const metaEl = $('gprogMeta'); if (metaEl) metaEl.textContent = '⏱ 总耗时 ' + elStr;
  setTimeout(() => $('gprog').classList.remove('show'), 2000);
}
function gErr(msg){
  $('gprogFill').style.width = '100%';
...
function showStep(step){
  ...
  // 确保当前步骤的默认子标签可见
  try {
    if (step === 'start' && typeof switchStartTab === 'function') {
      const activeTab = document.querySelector('.start-subtab.active');
      switchStartTab(activeTab ? activeTab.dataset.subtab : 'smart');
    }
    if (step === 'narrate' && typeof switchNarrTab === 'function') {
      const activeTab = document.querySelector('.narr-subtab.active');
      if (activeTab) switchNarrTab(activeTab.dataset.subtab);
    }
    if (step === 'music' && typeof switchMusicTab === 'function') {
      const activeTab = document.querySelector('.music-subtab.active');
      if (activeTab) switchMusicTab(activeTab.dataset.subtab);
    }
    if (step === 'adjust' && typeof switchAdjustTab === 'function') {
      const activeTab = document.querySelector('.adjust-subtab.active');
      if (activeTab) switchAdjustTab(activeTab.dataset.subtab);
    }
  } catch(e){ console.warn('子标签激活失败:', e); }
  // === 视频复用：进入有上传区的步骤时，如果该步骤尚无视频但全局已有，自动复用 ===
  try {
    if (CURRENT_VIDEO && CURRENT_VIDEO.file) {
      if (step === 'narrate' && !NAR_VIDEO) {
        setNarVideo(CURRENT_VIDEO.file, true);
      } else if (step === 'build' && !BC_VIDEO && $('bcDrop')) {
        setBCVideo(CURRENT_VIDEO.file, true);
      } else if (step === 'narrate' && !MOVIE_VIDEO && $('movieDrop')) {
        // movieCard 可能在同一 narrate 步骤中
        const activeTab = document.querySelector('.narr-subtab.active');
        if (activeTab && activeTab.dataset.subtab === 'movie') setMovieVideo(CURRENT_VIDEO.file, true);
      }
    }
  } catch(e){ console.warn('视频复用失败:', e); }
}
...
function loadTasks(){
  fetch('/api/tasks').then(r=>r.json()).then(res=>{
    ...
    list.innerHTML = tasks.map(t=>{
      const isQueued = !!t.queued && !t.done && !t.error;
      const isRunning = !isQueued && !t.done && !t.error;
      const isFailed = !!t.error;
      const pct = Math.min(100, Math.max(0, Number(t.pct)||0));
      // 排队中以独立颜色区分于「运行中」，避免与真正在跑的卡混淆
      let statusCls = isQueued ? 'task-queued' : (isRunning ? 'task-running' : (isFailed ? 'task-failed' : 'task-done'));
      let statusText = isQueued ? '📋 排队中' : (isRunning ? '⏳ 运行中' : (isFailed ? '❌ 失败' : '✅ 已完成'));
      ...
      if(isQueued){
        // 把后端给的「排队中（前面还有N个任务）」展示成更易读的简短文案
        const m = (phaseText||'').match(/前面还有(\d+)个/);
        phaseText = m ? ('等待中：前面还有 ' + m[1] + ' 个任务') : '等待空闲名额…';
      }
      if(isFailed && t.error){
        phaseText = '错误：' + String(t.error).slice(0,100);
      }
      // [2.2] 配音失败段对前端可见：任务卡展示「第X段配音失败，已用Y替换/该段静音」，杜绝静默丢段
      if(Array.isArray(t.tts_failures) && t.tts_failures.length){
        const notes = t.tts_failures.map(f=>{
          const who = f.engine ? ('已用 '+f.engine+' 替换') : '该段静音';
          return '第'+(Number(f.idx)+1)+'段配音失败('+(f.error||'未知原因')+')，'+who;
        }).join('；');
        phaseText = (phaseText ? phaseText+'｜' : '') + notes;
      }
      return '<div class="task-card '+statusCls+'" data-runid="'+t.runid+'">'
        + '<div class="task-head">'
...
function cancelTask(runid){
  // [P2-2] 区分队列 vs 运行中的二次确认文案：从当前已渲染的 tasksList 卡片读取
  // 状态，避免再发一次 /api/tasks 请求
  var card = document.querySelector('.task-card[data-runid="' + runid + '"]');
  var isQueued = !!(card && card.classList.contains('task-queued'));
  var msg = isQueued
    ? ('确定从队列中移除任务 ' + runid + '？\n（任务尚未开始，取消后下次提交需重新排队）')
    : ('确定取消任务 ' + runid + '？');
...
async function uploadChunksOnly(file){
  ...
  const _inflight = new Set();  // 跟踪所有在传的请求，失败时统一中止避免249/250假死
  ...
  async function fetchWithTimeout(url, opts, timeoutMs){
    ...
    finally { clearTimeout(timer); _inflight.delete(ctrl); }
  }
  function _abortAllInflight(){ _inflight.forEach(c=>{try{c.abort();}catch(e){}}); _inflight.clear(); }
  async function uploadOneChunk(i, attempt){
    if(failed) throw new Error('已取消');
      ...
      for(let attempt = 1; attempt <= 3 && !ok && !failed; attempt++){
        try {
          ...
          if(attempt < 3){
            ...
            failed = e;
            _abortAllInflight();  // 中止所有在传请求，避免249/250假死
          }
        }
      }
      if(ok && !failed){
...
// 内联 base64 上限（batch5-5.2）：≤8MB 的小文件维持 base64 JSON（少 3 个请求），
// 超过一律走分片上传——base64 让请求体膨胀 1.37 倍，服务端 json 解析 + b64 解码
// 还要各复制一份，60MB 视频内联的内存峰值约 320MB；分片路径每片仅 4MB 常驻。
const INLINE_UPLOAD_MAX = 8*1024*1024;

async function videoToBody(file){
  if(file.size <= INLINE_UPLOAD_MAX){
...
// 音乐同规则：≤8MB 内联 base64，超过走分片（后端 _resolve_music 支持 upload_id 形态）
async function musicToBody(file){
  if(file.size <= INLINE_UPLOAD_MAX){
    return { name:file.name, data: toB64(new Uint8Array(await file.arrayBuffer())) };
  }
  const uid = await uploadChunksOnly(file);
...
function setBCVideo(file, fromGlobal){
  if(!file.type.startsWith('video/')){ $('bcInfo').textContent='❌ 只支持视频'; return; }
  BC_VIDEO = file;
  $('bcDrop').textContent = '🎬 已选：' + file.name + (fromGlobal ? ' ↻来自素材' : '');
  if (!fromGlobal) setGlobalVideo(file);
...
async function buildBeatCut(){
  ...
  else { body.music = await musicToBody(MUSIC.file); }
  try{
  ...
  return new Promise(resolve=>{
    ...
    const iv=setInterval(()=>{
      fetch('/api/progress?run='+runid).then(r=>r.json()).then(p=>{
        ...
        gSet(p.pct, p.phase, p.tts_failures);
        if(p.done){
...
function setNarVideo(file, fromGlobal){
  if(!file.type.startsWith('video/')){ $('narInfo').textContent='❌ 只支持视频'; return; }
  NAR_VIDEO = file;
  $('narDrop').textContent = '🎞️ 已选：' + file.name + (fromGlobal ? ' ↻来自素材' : '');
  $('narInfo').textContent = '已选视频，点「生成解说」开始（默认免费配音）。';
  if (!fromGlobal) setGlobalVideo(file);  // 非来自全局时才同步
}
...
function setMovieVideo(file, fromGlobal){
  if(!file.type.startsWith('video/')){ $('movieInfo').textContent='❌ 只支持视频'; return; }
  MOVIE_VIDEO = file;
  $('movieDrop').textContent = '🎞️ 已选：' + file.name + (fromGlobal ? ' ↻来自素材' : '');
...
async function build(){
  ...
  for (const it of ITEMS){
    ...
    // 素材 >8MB 一律走分片上传（batch5-5.2：图片/视频同规则，控住 JSON 请求体体积
    // 与服务端内存峰值）；小素材维持 base64（少 3 个请求）
    if (it.file.size > INLINE_UPLOAD_MAX){
  ...
  if (MUSIC){
    ...
    else { mp = await musicToBody(MUSIC.file); }
    body.music = mp;
  }
  try {
...
function handleSmartVideo(file){
  const VIDEO_EXT = ['mp4','mkv','avi','mov','flv','wmv','webm','m4v','3gp','ts','rmvb','rm'];
  const ext = (file.name.split('.').pop() || '').toLowerCase();
  const isVideo = file.type.startsWith('video/') || VIDEO_EXT.indexOf(ext) >= 0;
  if(!isVideo){ $('smartStatus').textContent='❌ 请选择视频文件（支持mp4/mkv/avi/mov等）'; return; }
  SMART_VIDEO = file;
...
let _redoStack = [];
const MAX_UNDO = 50;
let _pendingSnapshot = null;    // 输入框聚焦时的临时快照（change 时才决定要不要入栈）
let _suppressUndoReset = false; // undo/redo 内部触发 renderAdjustPanel 时不清栈

function _cleanTtsText(s){
  return (s||'').replace(/\{(?:情绪|停顿|慢|快|高音|低音|大声|小声)[^}]*\}/g,'')
               .replace(/\{\/(?:情绪|停顿|慢|快|高音|低音|大声|小声)\}/g,'')
               .replace(/\s+/g,' ').trim();
}

function _snapshot(){ return JSON.stringify(_adjustState.items || []); }

/** 把「操作发生之前」的状态压入撤销栈，并清空重做栈。
 *  snapshotOverride：输入框场景传入聚焦那一刻的快照。 */
function _pushUndo(snapshotOverride){
  if(!_adjustState.items) return;
  var snap = (typeof snapshotOverride === 'string') ? snapshotOverride : _snapshot();
...
/** 丢弃刚压入的栈顶（拖拽了但数值没变时避免产生空撤销帧） */
function _dropUndoIfUnchanged(depth){
  if(typeof depth !== 'number') return;
  if(_undoStack.length === depth){ _undoStack.pop(); _updateUndoButtons(); }
}

// 输入框：聚焦时先拍快照，change 时才入栈（避免每敲一个字就入一帧）
function _snapshotOnFocus(){ if(_adjustState.items) _pendingSnapshot = _snapshot(); }

/** 比对当前输入框的 DOM 值与快照，判断是否真的改过 */
function _domMatchesSnapshot(snapStr){
  try{
    var snap = JSON.parse(snapStr);
    if(!Array.isArray(snap) || snap.length !== (_adjustState.items||[]).length) return false;
    for(var i=0;i<snap.length;i++){
      var t = document.getElementById('adjText'+i);
      if(t && (t.value||'').trim() !== _cleanTtsText(snap[i].text)) return false;
      var vs = document.getElementById('adjVStart'+i);
      if(vs && Math.abs((parseFloat(vs.value)||0) - (snap[i].video_start||0)) > 0.001) return false;
      var ve = document.getElementById('adjVEnd'+i);
      if(ve && Math.abs((parseFloat(ve.value)||0) - (snap[i].video_end||0)) > 0.001) return false;
    }
    return true;
  }catch(e){ return false; }
}

function _commitFocusSnapshot(){
  if(_pendingSnapshot === null) return;
  var s = _pendingSnapshot; _pendingSnapshot = null;
  if(!_domMatchesSnapshot(s)) _pushUndo(s);
}

/** 撤回后把 items 写回界面：段数变了必须重建列表 DOM，否则只回填输入框+重绘时间轴 */
function _applyItems(items){
  var domCount = document.querySelectorAll('#adjustList .adjust-item').length;
  if(domCount !== items.length){
    _suppressUndoReset = true;
    try{ renderAdjustPanel(items, _adjustState.runDir, _adjustState.mode, []); }
    finally{ _suppressUndoReset = false; }
    return;
  }
  _adjustState.items = items;
  _syncAdjustInputs();
  renderTimeline();
}

/** 用 items 回填片段列表输入框，保证撤销后 DOM 与数据一致 */
function _syncAdjustInputs(){
  var items = _adjustState.items || [];
  for(var i=0;i<items.length;i++){
    var it = items[i];
    var t = document.getElementById('adjText'+i);
    if(t) t.value = _cleanTtsText(it.text);
    var vs = document.getElementById('adjVStart'+i);
    if(vs) vs.value = (it.video_start !== undefined ? it.video_start : 0);
    var ve = document.getElementById('adjVEnd'+i);
    if(ve) ve.value = (it.video_end !== undefined ? it.video_end : 0);
  }
}

function _restoreHint(text){
  var hint = document.getElementById('adjustVideoHint');
  if(hint) hint.textContent = text;
}

function _updateUndoButtons(){
  var u = document.getElementById('btnUndoAdjust');
  var r = document.getElementById('btnRedoAdjust');
  if(u){
    u.disabled = (_undoStack.length === 0);
    u.style.opacity = u.disabled ? '0.4' : '1';
    u.style.cursor = u.disabled ? 'not-allowed' : 'pointer';
    u.title = u.disabled ? '没有可撤销的操作（Ctrl+Z）' : ('撤销（Ctrl+Z）· 还可撤销 '+_undoStack.length+' 次');
  }
  if(r){
    r.disabled = (_redoStack.length === 0);
    r.style.opacity = r.disabled ? '0.4' : '1';
    r.style.cursor = r.disabled ? 'not-allowed' : 'pointer';
    r.title = r.disabled ? '没有可重做的操作（Ctrl+Y）' : ('重做（Ctrl+Y）· 还可重做 '+_redoStack.length+' 次');
  }
}
...
function undoAdjust(){
  ...
  while(_redoStack.length > MAX_UNDO) _redoStack.shift();
  _applyItems(JSON.parse(_undoStack.pop()));
  _restoreHint('↩️ 已撤销（还可撤销 '+_undoStack.length+' 次，Ctrl+Y 重做）');
  _updateUndoButtons();
  scheduleAutoSave();
}
...
function redoAdjust(){
  ...
  while(_undoStack.length > MAX_UNDO) _undoStack.shift();
  _applyItems(JSON.parse(_redoStack.pop()));
  _restoreHint('↪️ 已重做（还可重做 '+_redoStack.length+' 次）');
  _updateUndoButtons();
  scheduleAutoSave();
}

/** 输入框变更入口：提交聚焦时的快照 → 写回 items → 重绘 */
function _onSegFieldChange(idx, field){
  _commitFocusSnapshot();
  var it = _adjustState.items[idx];
  if(!it) return;
  if(field === 'vs'){
    var v = parseFloat((document.getElementById('adjVStart'+idx)||{}).value);
    if(isFinite(v)) it.video_start = Math.max(0, Math.min(v, (it.video_end||0) - 0.1));
  } else if(field === 've'){
    var v2 = parseFloat((document.getElementById('adjVEnd'+idx)||{}).value);
    if(isFinite(v2)) it.video_end = Math.max((it.video_start||0) + 0.1, v2);
  } else if(field === 'text'){
    it.text = (document.getElementById('adjText'+idx)||{}).value || '';
  }
...
function renderAdjustPanel(ttsList, runDir, mode, script){
  // 重载入列表 = 新的编辑基线，清空撤销/重做栈（undo/redo 内部重建时跳过）
  if(!_suppressUndoReset){ _undoStack = []; _redoStack = []; _pendingSnapshot = null; }
  var _keepVideoDur = _adjustState.videoDuration;
  _adjustState = { runDir: runDir, mode: mode, items: JSON.parse(JSON.stringify(ttsList)), changed: {} };
  if(_keepVideoDur) _adjustState.videoDuration = _keepVideoDur;  // 跨重绘保留源片时长
  ...
  ttsList.forEach(function(item, idx){
    ...
    html += '</div>';
    html += '<textarea id="adjText'+idx+'" rows="2" style="width:100%;padding:8px;border-radius:6px;border:1px solid var(--border);background:var(--bg);color:var(--text);font-size:13px;resize:vertical;box-sizing:border-box" onfocus="_snapshotOnFocus()" onchange="_onSegFieldChange('+idx+',\'text\')" oninput="_adjustState.changed['+idx+']=true">'+cleanText.replace(/</g,'&lt;').replace(/>/g,'&gt;')+'</textarea>';
  ...
  // 显示标签栏
  var tabs = document.getElementById('adjustTabs');
  if(tabs) tabs.style.display = 'flex';
  // 默认显示预览剪辑tab
  switchAdjustTab('preview');
  // 保存视频时长（从tts_list或探测）
  ...
  // 显示项目工具栏，加载最近项目
  var ptb = document.getElementById('projectToolbar');
  ...
  if(pn && !pn.value){
    var d = new Date();
    pn.value = '项目_' + d.getFullYear() + (d.getMonth()+1) + d.getDate() + '_' + d.getHours() + d.getMinutes();
  }
  autoSaveProject();
  // 绑定确认按钮
  ...
  if(!window._adjKeyBound){
    ...
    document.addEventListener('keydown', function(e){
      // 只在手动调整卡片激活时响应（卡片用 .active 控制显隐，页面里没有 adjustPanel 节点）
      var card = document.getElementById('adjustCard');
      if(!card || !card.classList.contains('active')) return;
      if(!_adjustState.items || _adjustState.items.length === 0) return;
      // 输入框中不响应快捷键
...
function renderTimeline(){
  ...
  if(!isFinite(totalDur) || totalDur <= 0) totalDur = 60;  // NaN保护：避免所有片段缩成0
  ...
  items.forEach(function(it, idx){
    ...
    const isBroll = !!it.isBroll;
    const c = isBroll ? '#6b7280' : colors[idx % colors.length];
    const borderStyle = isBroll ? '2px dashed rgba(255,255,255,0.5)' : '1px solid rgba(255,255,255,0.2)';
    let warn = '';
    if(!isBroll && vdur < adur - 0.3){ warn = ' ⚠️视频短'; }
    else if(!isBroll && vdur > adur + 1){ warn = ' ⚠️视频长'; }
    const segLabel = isBroll ? '🎬 B-roll' : '第'+(idx+1)+'段';
    vHtml += '<div class="tl-vseg" data-idx="'+idx+'" style="position:absolute;left:'+left+'%;top:3px;width:'+w+'%;height:32px;background:'+c+';border-radius:4px;overflow:hidden;opacity:0.75;border:'+borderStyle+'">';
  ...
  items.forEach(function(it, idx){
    if(it.isBroll){ cumVideoForAudio += Math.max(0, (it.video_end||0)-(it.video_start||0)); return; }
...
function tlStartResize(e){
  ...
  _tlDrag = {
    ...
    undoDepth: _undoStack.length,   // 若最终没改动则丢弃这一帧
    dirty: false
  };
...
function tlDoResize(e){
  ...
  if(_tlDrag.side === 'l'){
    ...
    it.video_end = Math.min(_tlDrag.videoDur, newEnd2);
  }
  // 显示磁吸辅助线
...
function tlDoAudioDrag(e){
  ...
  if(Math.abs((it.audio_offset||0) - _tlDrag.origOffset) > 1e-6) _tlDrag.dirty = true;
  renderTimeline();
}
...
// 一键对齐：视频段长度匹配配音时长
function alignSegmentToAudio(idx, noPushUndo){
  var it = _adjustState.items[idx];
  if(!it || !it.duration) return;
  if(!noPushUndo) _pushUndo();
...
function alignAllToAudio(){
  if(!_adjustState.items) return;
  _pushUndo();   // 整体对齐只记一帧，撤销一次即可全部还原
  for(var i=0;i<_adjustState.items.length;i++){
    alignSegmentToAudio(i, true);
  }
}
...
function isBuiltinTemplate(name){ return name && name.startsWith('⭐ '); }
function loadPresetList(){
  ...
  builtinGroup.label = '⭐ 官方模板';
  Object.keys(BUILTIN_TEMPLATES).forEach(name=>{
    const opt = document.createElement('option');
    opt.value = name; opt.textContent = name;
    opt.title = BUILTIN_TEMPLATES[name].desc;
...
function applyPreset(name){
  ...
  if(p.targetSec && document.getElementById('narTargetSec')) document.getElementById('narTargetSec').value = p.targetSec;
  if(p.subtitle){
...
async function recommendSegments(idx){
  ...
  try{
    ...
    out.candidates.forEach(function(c, i){
      ...
      var srcLabel = c.source === '画面' ? '🎬画面' : '💬台词';
      var srcColor = c.source === '画面' ? '#06b6d4' : '#6366f1';
      html += '<span style="background:'+srcColor+';color:#fff;padding:1px 6px;border-radius:3px;font-size:11px;white-space:nowrap">'+srcLabel+'</span>';
...
async function regenSingleTts(idx){
  ...
  try{
    ...
    if(result.audio || result.duration) _pushUndo();
...
// === 项目文件管理（.framecut）===
const PROJECT_AUTOSAVE_KEY = 'framecut_autosave';
const PROJECT_RECENT_KEY = 'framecut_recent';

function collectProjectData(){
  if(!_adjustState || !_adjustState.items) return null;
  var items = _adjustState.items.map(function(it, idx){
    return {
      index: idx,
      text: it.text || '',
      audio: it.audio || '',
      duration: it.duration || 0,
      video_start: it.video_start || 0,
      video_end: it.video_end || 0,
      audio_offset: it.audio_offset || 0,
      skip: !!it.skip
    };
  });
  var subStyle = {};
  ['subFontSize','subColor','subOutlineColor','subOutlineWidth','subPosition','subMarginBottom'].forEach(function(id){
  ...
  return {
    ...
    video_path: _adjustState.videoPath || '',
    run_dir: _adjustState.runDir || '',
    items: items,
    settings: {
      subtitle: subStyle,
      export: exportSet,
      autoCut: (document.getElementById('narAutoCut') || {}).checked || true
    }
  };
}
...
function saveProject(){
  ...
  a.href = url;
  a.download = (data.name || '项目') + '.framecut';
  document.body.appendChild(a);
  a.click();
  document.body.removeChild(a);
  URL.revokeObjectURL(url);
  addToRecentProjects(data);
...
function loadProject(file){
  ...
  reader.onload = function(e){
    try{
      ...
      var pn = document.getElementById('projectName');
      if(pn) pn.value = data.name || '未命名项目';
      var items = (data.items || []).map(function(it){
        return {
          text: it.text || '',
          audio: it.audio || '',
          duration: it.duration || 3,
          video_start: it.video_start || 0,
          video_end: it.video_end || 0,
          audio_offset: it.audio_offset || 0,
          skip: !!it.skip
      ...
      if(data.settings){
        if(data.settings.subtitle){
          Object.keys(data.settings.subtitle).forEach(function(id){
            var el = document.getElementById(id);
            if(el) el.value = data.settings.subtitle[id];
          });
        }
        if(data.settings.export){
          Object.keys(data.settings.export).forEach(function(id){
            var el = document.getElementById(id);
            if(el) el.value = data.settings.export[id];
          });
        }
      }
      renderAdjustPanel(items, data.run_dir || '', 'movie', []);
      var preview = document.getElementById('adjustVideoPreview');
      var timeline = document.getElementById('adjustTimeline');
      if(preview) preview.style.display = 'block';
      if(timeline) timeline.style.display = 'block';
      if(!_adjustState.videoDuration){
        var maxEnd = 0;
        items.forEach(function(it){ if(it.video_end > maxEnd) maxEnd = it.video_end; });
        if(maxEnd > 0) _adjustState.videoDuration = Math.ceil(maxEnd / 60) * 60 + 60;
      }
      renderTimeline();
      ensureAdjVideoLoaded();
      addToRecentProjects(data);
      var status = document.getElementById('adjustStatus');
      if(status) status.textContent = '已加载项目：' + (data.name || '') + '（' + items.length + '段）';
      var hint = document.getElementById('projectAutoSaveHint');
...
function autoSaveProject(){
  try{
    var data = collectProjectData();
    if(!data) return;
    localStorage.setItem(PROJECT_AUTOSAVE_KEY, JSON.stringify(data));
    var hint = document.getElementById('projectAutoSaveHint');
    if(hint) hint.textContent = '自动保存 ' + new Date().toLocaleTimeString();
  }catch(e){}
}
...
function addToRecentProjects(data){
  try{
    var recent = JSON.parse(localStorage.getItem(PROJECT_RECENT_KEY) || '[]');
    recent = recent.filter(function(p){ return !(p.name === data.name && p.created === data.created); });
    recent.unshift({
      name: data.name,
    ...
    localStorage.setItem(PROJECT_RECENT_KEY, JSON.stringify(recent));
    loadRecentProjects();
  }catch(e){}
}
...
function loadRecentProjects(){
  ...
  try{
    ...
    listEl.innerHTML = recent.map(function(p, i){
      return '<button class="btn-secondary" style="padding:4px 10px;font-size:11px;white-space:nowrap" onclick="loadRecentProject(' + i + ')">' +
             '最近 ' + (p.name || '未命名') + ' · ' + (p.itemCount || 0) + '段' +
             '</button>';
    }).join('');
...
function loadRecentProject(idx){
  try{
    var autoData = JSON.parse(localStorage.getItem(PROJECT_AUTOSAVE_KEY) || 'null');
    if(autoData){
      var fakeFile = new Blob([JSON.stringify(autoData)], {type: 'application/json'});
...
const SFX_LIBRARY = {
  '转场': [
    {name: '嗖~ Whoosh', fn: 'whoosh', dur: 0.8},
    {name: '呼啸 Swoosh', fn: 'swoosh', dur: 1.0},
    {name: '扫频 Sweep', fn: 'sweep', dur: 1.2},
    {name: '故障 Glitch', fn: 'glitch', dur: 0.5},
    {name: '倒带 Rewind', fn: 'rewind', dur: 1.0},
    {name: '磁带停 Tape Stop', fn: 'tapeStop', dur: 0.8},
  ],
  '强调': [
    {name: '砰 Pop', fn: 'pop', dur: 0.2},
    {name: '轰 Boom', fn: 'boom', dur: 0.6},
    {name: '叮 Ding', fn: 'ding', dur: 0.5},
    {name: '撞击 Impact', fn: 'impact', dur: 0.4},
    {name: '上升 Riser', fn: 'riser', dur: 1.5},
    {name: '下降 Faller', fn: 'faller', dur: 1.0},
  ],
  '氛围': [
    {name: '氛围垫 Pad', fn: 'ambientPad', dur: 2.0},
    {name: '低音 Drone', fn: 'drone', dur: 2.0},
    {name: '悬疑 Tension', fn: 'tension', dur: 1.5},
    {name: '黑胶杂音 Vinyl', fn: 'vinyl', dur: 2.0},
  ],
  'UI/提示': [
    {name: '点击 Click', fn: 'click', dur: 0.1},
    {name: '打字 Typewriter', fn: 'typewriter', dur: 0.8},
    {name: '通知 Notification', fn: 'notification', dur: 0.4},
    {name: '成功 Success', fn: 'success', dur: 0.5},
    {name: '错误 Error', fn: 'errorBuzz', dur: 0.3},
  ]
};

function generateSfxBuffer(fnName, dur){
  var ctx = getSfxCtx();
  ...
  for(var i=0; i<len; i++){
    ...
    switch(fnName){
      case 'whoosh':
        ...
        var freq = 400 + 800 * (t/dur);
        var filter = Math.sin(Math.PI * t/dur);
        val = noise * filter * 0.3 * Math.exp(-3*t/dur);
        break;
      case 'swoosh':
        var noise = Math.random()*2-1;
        var env = Math.sin(Math.PI * t/dur);
        val = noise * env * 0.25 * (0.5 + 0.5*Math.sin(20*t));
        break;
      case 'sweep':
        var freq = 200 + 2000 * (t/dur);
        val = Math.sin(2*Math.PI*freq*t) * Math.sin(Math.PI*t/dur) * 0.2;
        break;
      case 'glitch':
        var noise = Math.random()*2-1;
        var gate = (Math.sin(40*t) > 0) ? 1 : 0.1;
        val = noise * gate * 0.2 * Math.exp(-2*t/dur);
        break;
      case 'rewind':
        var noise = Math.random()*2-1;
        var speed = 1 + 3*(1-t/dur);
        val = noise * Math.sin(50*t*speed) * 0.15;
        break;
      case 'tapeStop':
        var freq = 440 * (1 - 0.8*t/dur);
        val = Math.sin(2*Math.PI*freq*t) * Math.exp(-4*t/dur) * 0.2;
        break;
      case 'pop':
        var freq = 600 * Math.exp(-15*t);
        val = Math.sin(2*Math.PI*freq*t) * Math.exp(-20*t) * 0.4;
        break;
      case 'boom':
        var freq = 80 + 40*Math.exp(-5*t);
        val = Math.sin(2*Math.PI*freq*t) * Math.exp(-3*t) * 0.4;
        val += (Math.random()*2-1) * Math.exp(-8*t) * 0.2;
        break;
      case 'ding':
        var freq = 1200;
        val = Math.sin(2*Math.PI*freq*t) * Math.exp(-5*t) * 0.25;
        val += Math.sin(2*Math.PI*freq*1.5*t) * Math.exp(-8*t) * 0.15;
        break;
      case 'impact':
        var noise = Math.random()*2-1;
        val = noise * Math.exp(-15*t) * 0.4;
        val += Math.sin(2*Math.PI*100*t) * Math.exp(-10*t) * 0.3;
        break;
      case 'riser':
        var freq = 100 + 1000*(t/dur);
        val = Math.sin(2*Math.PI*freq*t) * (t/dur) * 0.15;
        val += (Math.random()*2-1) * (t/dur) * 0.05;
        break;
      case 'faller':
        var freq = 1000 - 900*(t/dur);
        val = Math.sin(2*Math.PI*Math.max(50,freq)*t) * (1-t/dur) * 0.15;
        break;
      case 'ambientPad':
        val = (Math.sin(2*Math.PI*220*t) + Math.sin(2*Math.PI*277*t) + Math.sin(2*Math.PI*330*t)) * 0.08;
        val *= 0.5 + 0.5*Math.sin(0.5*t);
        break;
      case 'drone':
        val = Math.sin(2*Math.PI*55*t) * 0.15 + Math.sin(2*Math.PI*55.5*t) * 0.1;
        val *= Math.sin(Math.PI*t/dur);
        break;
      case 'tension':
        var freq = 80 + 20*Math.sin(5*t);
        val = Math.sin(2*Math.PI*freq*t) * 0.12;
        val += (Math.random()*2-1) * 0.02;
        break;
      case 'vinyl':
        val = (Math.random()*2-1) * 0.04;
        if(Math.random() < 0.001) val += (Math.random()*2-1) * 0.3;
        break;
      case 'click':
        val = (Math.random()*2-1) * Math.exp(-50*t) * 0.3;
        break;
      case 'typewriter':
        var pos = (t/dur) * 8;
        var charPos = pos - Math.floor(pos);
        if(charPos < 0.1){
          val = (Math.random()*2-1) * Math.exp(-30*charPos) * 0.25;
        }
        break;
      case 'notification':
        val = Math.sin(2*Math.PI*880*t) * Math.exp(-8*t) * 0.2;
        if(t > 0.15) val += Math.sin(2*Math.PI*1100*t) * Math.exp(-8*(t-0.15)) * 0.2;
        break;
      case 'success':
        var notes = [523, 659, 784];
        var ni = Math.floor(t/dur * 3);
        if(ni < 3) val = Math.sin(2*Math.PI*notes[ni]*t) * Math.exp(-5*(t% (dur/3))) * 0.2;
        break;
      case 'errorBuzz':
        val = Math.sin(2*Math.PI*200*t) * (Math.sin(20*t) > 0 ? 0.2 : 0.05);
        break;
      default:
        val = 0;
    }
...
function playSfx(fnName, dur){
  var ctx = getSfxCtx();
  var buf = generateSfxBuffer(fnName, dur);
...
function downloadSfx(name, fnName, dur){
  var ctx = getSfxCtx();
  var buf = generateSfxBuffer(fnName, dur);
  // 编码为WAV
  ...
  var writeStr = function(o, s){ for(var i=0;i<s.length;i++) dv.setUint8(o+i, s.charCodeAt(i)); };
  ...
  dv.setUint16(34, 16, true); writeStr(36, 'data'); dv.setUint32(40, dataLen, true);
  var offset = 44;
  for(var c=0; c<numCh; c++){
    var ch = buf.getChannelData(c);
    for(var i=0; i<len; i++){
      dv.setInt16(offset + (i*numCh+c)*2, Math.max(-1,Math.min(1,ch[i]))*32767, true);
    }
  }
  var blob = new Blob([ab], {type: 'audio/wav'});
  var url = URL.createObjectURL(blob);
  var a = document.createElement('a');
  a.href = url; a.download = name.replace(/[^a-zA-Z0-9]/g,'_') + '.wav';
  document.body.appendChild(a); a.click(); document.body.removeChild(a);
  URL.revokeObjectURL(url);
}

var _sfxCurrentCat = '转场';
function renderSfxLibrary(){
  var catEl = document.getElementById('sfxCategories');
  var gridEl = document.getElementById('sfxGrid');
  if(!catEl || !gridEl) return;
  // 分类按钮
  catEl.innerHTML = Object.keys(SFX_LIBRARY).map(function(cat){
    var active = cat === _sfxCurrentCat ? 'background:var(--accent);color:#fff' : '';
    return '<button class="btn-secondary" style="padding:4px 12px;font-size:12px;'+active+'" onclick="selectSfxCategory(\''+cat+'\')">'+cat+'</button>';
  }).join('');
  // 音效网格
  var items = SFX_LIBRARY[_sfxCurrentCat] || [];
  gridEl.innerHTML = items.map(function(sfx){
    return '<div style="padding:10px;border-radius:8px;background:var(--bg);border:1px solid var(--border);text-align:center">' +
           '<div style="font-size:12px;font-weight:600;margin-bottom:6px">'+sfx.name+'</div>' +
           '<div style="display:flex;gap:6px;justify-content:center">' +
           '<button class="btn-secondary" style="padding:4px 10px;font-size:11px" onclick="playSfx(\''+sfx.fn+'\','+sfx.dur+')">▶️ 试听</button>' +
           '<button class="btn-secondary" style="padding:4px 10px;font-size:11px" onclick="downloadSfx(\''+sfx.name+'\',\''+sfx.fn+'\','+sfx.dur+')">💾 下载</button>' +
           '</div></div>';
  }).join('');
}

function selectSfxCategory(cat){
  _sfxCurrentCat = cat;
  renderSfxLibrary();
}

function toggleSfxLibrary(){
  var grid = document.getElementById('sfxGrid');
  var cats = document.getElementById('sfxCategories');
  if(grid && cats){
    var hidden = grid.style.display === 'none';
    grid.style.display = hidden ? '' : 'none';
    cats.style.display = hidden ? '' : 'none';
  }
}


// === AI B-roll 建议 ===
function analyzeBroll(){
  var items = _adjustState.items;
  ...
  if(!resultsEl) return;
  // 从runDir提取run_id
  var runId = runDir.split(/[\\/]/).pop() || runDir;
  resultsEl.innerHTML = '<div style="padding:10px">🔍 正在分析 '+items.length+' 段解说词的B-roll建议…</div>';
  var allResults = [];
  var done = 0;
  items.forEach(function(it, idx){
    if(!it.text || it.text.length < 4){ done++; return; }
    fetch('/api/recommend_segments', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({run_id: runId, text: it.text})
    }).then(function(r){ return r.json(); }).then(function(data){
      done++;
      if(data.ok && data.candidates && data.candidates.length){
        allResults.push({idx: idx, text: it.text.substring(0,40), candidates: data.candidates});
      }
      if(done >= items.length){
        renderBrollResults(allResults);
      }
    }).catch(function(){
      done++;
      if(done >= items.length) renderBrollResults(allResults);
    });
  });
}

function renderBrollResults(results){
  var el = document.getElementById('brollResults');
  if(!el) return;
  if(!results.length){
    el.innerHTML = '<div style="padding:8px;color:var(--muted)">未找到B-roll建议（可能无ASR台词数据，或解说词与原片台词重叠较少）。</div>';
    return;
  }
  var html = '<div style="margin-bottom:8px;color:#10b981">✅ 找到 '+results.length+' 段有B-roll建议</div>';
  results.forEach(function(r){
    html += '<div style="margin:8px 0;padding:8px;border-radius:6px;background:var(--bg);border:1px solid var(--border)">';
    html += '<div style="font-size:12px;font-weight:600;margin-bottom:4px">段'+(r.idx+1)+': "'+r.text+'…"</div>';
    r.candidates.forEach(function(c, ci){
...
function insertBroll(afterIdx, start, end){
  ...
  items.splice(afterIdx + 1, 0, broll);
  renderAdjustPanel(items, _adjustState.runDir, _adjustState.mode, []);
  var status = document.getElementById('adjustStatus');
  if(status) status.textContent = '🎬 已在段'+(afterIdx+1)+'后插入B-roll（'+start+'s-'+end+'s），可在时间轴调整';
  scheduleAutoSave();
}
...
function switchAdjustTab(tabName){
  var tabs = document.querySelectorAll('.adjust-tab');
  var contents = document.querySelectorAll('.adjust-tab-content');
  ...
  contents.forEach(function(c){
    c.style.display = (c.id === 'tab' + tabName.charAt(0).toUpperCase() + tabName.slice(1)) ? '' : 'none';
  });
  // 切换到高级工具时渲染音效库
  if(tabName === 'advanced'){
    renderSfxLibrary();
...
// === 解说页子标签切换 ===
function switchNarrTab(tabName){
  var tabs = document.querySelectorAll('.narr-subtab');
  var contents = document.querySelectorAll('.narr-tab-content');
  tabs.forEach(function(t){
    if(t.getAttribute('data-subtab') === tabName){
      t.classList.add('active');
      t.style.color = 'var(--accent)';
  ...
  // 切换子标签时：如果目标标签需要视频且尚无，从全局复用
  try {
    if (CURRENT_VIDEO && CURRENT_VIDEO.file) {
      if (tabName === 'movie' && !MOVIE_VIDEO) setMovieVideo(CURRENT_VIDEO.file, true);
      else if (tabName === 'nar' && !NAR_VIDEO) setNarVideo(CURRENT_VIDEO.file, true);
    }
  } catch(e){}
}
...
function switchMusicTab(tabName){
  var tabs = document.querySelectorAll('.music-subtab');
  var contents = document.querySelectorAll('.music-tab-content');
  tabs.forEach(function(t){
    if(t.getAttribute('data-subtab') === tabName){
      t.classList.add('active');
      t.style.color = 'var(--accent)';
...
// === 流程引导系统 ===
const FLOW_STEPS = [
  {num:1, step:'start', label:'开始', card:'smartCard'},
  {num:2, step:'upload', label:'素材', card:'drop'},
  {num:3, step:'narrate', label:'解说', card:'narCard'},
  {num:4, step:'adjust', label:'调整', card:'adjustCard'},
  {num:5, step:'build', label:'合成', card:'buildCard'}
];
// _flowCurrent 和 _flowDone 已在顶部声明（避免TDZ报错）

function goFlowStep(n){
  if(n < 1 || n > FLOW_STEPS.length) return;
  var s = FLOW_STEPS[n-1];
  _flowCurrent = n;
  // 跳转到对应卡片
  if(s.card && typeof goStep === 'function'){
    goStep(s.card);
  }
  updateFlowUI();
}
...
function updateFlowUI(){
  ...
  steps.forEach(function(el, idx){
    ...
    if(n < _flowCurrent || _flowDone.has(n)) el.classList.add('done');
    if(n === _flowCurrent) el.classList.add('active');
  });
  lines.forEach(function(el, idx){
    if(idx + 1 < _flowCurrent || _flowDone.has(idx+1)) el.classList.add('done');
...
function nextFlowStep(){
  markFlowDone(_flowCurrent);
  if(_flowCurrent < FLOW_STEPS.length){
    goFlowStep(_flowCurrent + 1);
  }
}

function toggleFlowMore(){
  var menu = document.getElementById('flowMoreMenu');
...
document.addEventListener('click', function(e){
  var menu = document.getElementById('flowMoreMenu');
  var btn = document.querySelector('.flow-more-btn');
