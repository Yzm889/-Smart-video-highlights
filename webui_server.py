...
def _vlm_sample_timeline(video_path, vdur, asr, run_dir, progress=None, interval=28.0):
    ...
    if run_dir and not os.path.isabs(run_dir):
        run_dir = os.path.join(OUTDIR, run_dir)
...
def _generate_all_tts(narr, run_dir, progress=None):
    ...
    # 转成绝对路径（基于 OUTDIR）避免 TTS 文件写到项目根目录导致"有日志无文件"。
    if run_dir and not os.path.isabs(run_dir):
        run_dir = os.path.join(OUTDIR, run_dir)
    results = []
    _tcfg = load_ai_config().get('tts') or {}
    ...
    skipped = 0
    _probe_counter = 0
    _log = []   # 诊断日志：[{idx, engine, ok, error, duration_ms, source}]
    for i, txt in enumerate(narr):
        if _aborted():
            break
        if not txt.strip():
        ...
        if os.path.exists(existing) and os.path.getsize(existing) > 100:
            ...
            skipped += 1
            _log.append({'idx': i, 'engine': 'cache', 'ok': True, 'error': '',
        ...
        if clip is not None:
            ...
            _log.append({'idx': i, 'engine': _eng, 'ok': True, 'error': '', 'duration_ms': _dur, 'source': 'fresh'})
        else:
            _log.append({'idx': i, 'engine': _eng, 'ok': False, 'error': _err, 'duration_ms': _dur, 'source': 'fresh'})
        if progress:
    ...
    if failed:
        ...
        for i in failed:
            ...
            if clip is not None:
                results.append((i, clip))
                _log.append({'idx': i, 'engine': _eng, 'ok': True, 'error': '',
                             'duration_ms': _dur, 'source': 'retry'})
            else:
                _log.append({'idx': i, 'engine': _eng, 'ok': False, 'error': _err,
                             'duration_ms': _dur, 'source': 'retry'})
            if progress:
    ...
    try:
        _bad = [e for e in _log if not e.get('ok')]
        _jd = {'segments': _log,
               'summary': {'total': len(narr), 'success': len(results),
                           'failed': len(narr) - len(results) - skipped,   # [2.2] 计数（兼容旧字段）
                           'skipped': skipped,
                           'engines_used': list(set(e.get('engine', '') for e in _log if e.get('engine'))),
