...
- **处理链（本地为主）**：
  ...
  3. **剧情解说生成（剧情旁白，不是画面描述）**：本地 VLM 先多帧理解整段剧情（人物/事件/台词）→ 生成**连贯的电影解说稿**——开场引入 + 剧情推进，面向观众讲"发生了什么"；智能模式用云端 LLM 基于台词+画面线索写剧情解说。**省流模式**用离线模板，0 元跑通。
  4. **时间轴**：解说稿按镜头段时间轴自动编号。
  5. **成片**：按时间轴配音（省流用 Windows 免费 SAPI 中文 TTS；智能用 MiMo 付费 TTS）+ 原声压低 + 可选**配乐**（勾选「加配乐」）+ 烧录解说字幕。
- **产出**：输入短视频 → 输出「带 AI 剧情解说 + 字幕」的解说视频。
- **成本**：本地 ASR 免费；LLM 按分钟计费（几毛/分钟），省流模式 0 元。
...
- **LLM 配置**：解说/剧情类纯文本走 `ai_config.chat` 段（见 `ai_config.example.json`），未配 `chat` 时自动回退 `vision` 段（兼容旧配置）；画面描述仍走 `vision`。省流模式（无 Key 或指令含「省流」）用离线切句，0 元跑通。
...
- **省流优先**：指令默认走免费路径；明确含「真AI / 智能 / 花钱」才调用付费接口；含「竖屏/横屏」自动设分辨率。
...
- **合并过碎镜头段**：影视片段常被切成一堆 4~8 秒短镜头，逐段解说必然重复；现在先合并成 ≤6 个"剧情环节"，每环节一句解说，更贴近真人解说节奏。
...
- **关键词（本地/云端两条路径都支持）**：
  - 本地省流：`ai_config.json` 配 `vlm`（如 ollama qwen2.5vl），0 元跑剧情解说；
