...
一个**本地运行、AI 智能**的短视频创作工具。上传图片 / 视频 + 选一首音乐，自动生成**强卡点**短片——画面切换点精确匹对到音乐鼓点；并内置完整的**电影剧情解说**流水线（场景分段 → AI 写连贯解说稿 → 配音 → 字幕 → 混音出片），支持 AI 看图写文案 + 中文配音 + 字幕烧录。
...
- 🎯 **智能强卡点**：分析视频的**场景切换 / 大幅动作停顿帧**，匹对音乐**大鼓点**，自动硬切 + 配乐成片（xfade 交叉淡化 / 保留原声 / 节拍对齐可调 0.5 / 1 / 2 拍）
- 🖼️ **多素材混排**：图片（自动 Ken Burns 镜头运动）+ 视频 自由组合
- 🎵 **免费踩点音乐库**：内置 CC.BY 商用曲库，在线搜索、一键使用
- 🎬 **电影解说流水线**：场景分段 → Whisper 台词 → **整稿生成连贯剧情解说**（黄金 7 秒钩子 / 画面接地 / 少升华 / 留白）→ 配音 → 字幕 → 混音出片；支持「剧情驱动剪辑」（写稿 → 按字数分配画面，成片短于原片）
- 🌐 **联网解说**：按片名搜索剧情 → 全自动解说（离线切句兜底，0 元可用）
- 💬 **指令成片**：自然语言一句话路由到对应工作流（解说 / 卡点 / 合成）
- 🎛 **人机协同微调**：方案预览面板（时间轴拖拽 / 删切点 / 改解说词 / B-roll 插入 / 单段配音重生成）+ 50 步撤销重做
- 🤖 **AI 文案 + 字幕 + 中文配音**：支持 DeepSeek（看图写话）、小米 MiMo / 通义（配音），可分别配置
- 💸 **自动选路（免费优先）**：本地模型优先 → 配了云端 Key 才用云端 → 都没有时用离线模板兜底。**不配任何 Key 也能全程免费出片**
- 🧠 **本地模型选择卡**：写稿三选一（`qwen3:14b` ⭐ / `qwen2.5:14b` / `qwen3:8b`）+ 看图二选一（`qwen3-vl:8b` ⭐ / `qwen2.5vl`），网页里点卡片即下载启用；Qwen3 思考段自动剥离，不污染解说稿
- ⚡ **国内加速通道**：网页拉取自动走 **hf-mirror + aria2c 多线程**（白名单 qwen3:14b-q4_K_M / qwen3-vl:8b 双文件 / qwen2.5 系），几 GB 约 2~3 分钟；失败自动回退官方源
- ⚡ **GPU 编码加速**：自动探测 `h264_nvenc` 硬编（约 **1.8× 提速**），不可用自动回退 `libx264`；三档画质（速度 / 均衡 / 画质）可选
- 🔇 **本地配音引擎**：edge-tts（免 Key·多音色）/ sherpa-onnx 离线模型 / 系统 SAPI 三选一 + 自动回退 + 熔断
- ⚡ **上传后后台预热**：上传完成即在空闲期预分析 ASR+VLM，点击解说时命中缓存秒出
- 🎙 **配音并发生成**：edge 本地配音 3 路并发（断点续跑 / 失败重试 / 熔断探测），50 段约 1 分钟
- 🧮 **GPU 资源隔离**：本地推理全局互斥（ASR / LLM / VLM 同槽串行，远端 API 可并行）——多任务并发不互抢 Ollama
- 🧠 **LLM 轮次合并 + 轻量模型**：语义对齐+润色一次调用；题材判型 / 详略 / 剧情理解走 8B 轻量模型（写稿、润色保留主模型不降级）
- 🗂 **本地素材库 + B 站集成**：`material_library/` 持久化素材；B 站关键词搜索 → 选片 → 下载 MP4 → 一键入库
- 🖼 **自动封面生成**：智能选帧（对比度+边缘能量打分）+ 大字标题合成封面，三种版式可换帧
- 🧹 **磁盘自动清理**：run_dir / 预热目录 / 上传残片纳入 6h 周期自动回收；分析缓存按「指纹+参数+版本」落盘（200 条上限自动清最旧）
- 🕘 最近生成历史、可取消合成（ffmpeg 立即终止 + 协作式标志）、竖屏发布规格、多种转场
- 🧪 **回归保障**：全量 **308 个测试**（0 失败）+ pyflakes 零告警门禁 + GitHub Actions CI（Python 3.9 / 3.11 双矩阵）

## 📁 目录结构

```
.
├── webui_server.py        # 后端入口：HTTP 服务 + 路由表 + 后台调度（进度/清扫线程）
├── handler.py             # HTTP 端点处理（do_GET/do_POST 路由表化，端点=方法+登记一行）
├── workflows.py           # 任务工作流与队列（dispatch_* / 持久化队列）
├── movie_narrator.py      # 电影解说：写稿/语义对齐/剧情理解/VLM 抽帧分析（断点续跑）
├── video_render.py        # 渲染：切片/转场/微变速/混音/字幕烧录/单遍编码
├── beat_analysis.py       # 强卡点：场景切分/节拍分析/抽帧/切点规划
├── ffmpeg_utils.py        # ffmpeg 封装（整体超时/协作取消/探针/进程回收）
├── cache_utils.py         # 分析缓存（指纹+参数+版本落盘，200 条上限）
├── ai_providers.py        # AI 接入：本地 Ollama / 云端多供应商 / hf-mirror 加速通道
├── tts_engines.py         # 配音引擎：edge-tts / sherpa-onnx / SAPI / 云端 / 并发调度
├── text_utils.py          # 文本工具（解说稿清洗/切句/对齐）
├── bili_downloader.py     # B 站素材：搜索/下载（playurl 直连 + yt-dlp 兜底）
├── static/                # 前端页面（index.html / style.css / app.js）
├── tests/                 # 回归测试（308 用例，pytest）
├── .github/workflows/     # CI：pyflakes 门禁 + pytest（Python 3.9/3.11 双矩阵）
├── ai_config.example.json # AI 配置模板（复制为 ai_config.json 使用）
├── requirements.txt       # Python 运行依赖
├── requirements-dev.txt   # 开发/回归依赖（pytest + pyflakes）
├── Dockerfile             # 容器镜像（含 ffmpeg / 中文字体 / HEALTHCHECK）
├── docker-compose.yml     # 一键部署（产物/曲库/配置挂载持久化）
├── material_library/      # 本地素材库（.gitignore 排除）
...
└── img1~4.png / demo.gif  # 内置春景示例图与演示
```
...
## 🎛️ PR式手动调整时间轴（Phase 5+）✅ 已就绪

生成配音后进入「手动调整」页面，提供类似 Premiere 的时间轴编辑器：

### 核心功能
- **视频轨**：每个解说段对应一个彩色色块，宽度=视频取片时长，可左右拖动调整位置
- **配音轨**：音频块可左右拖动对齐画面，虚线边框提示音画是否匹配
- **裁剪**：色块左右手柄调整视频入点/出点，实时预览
- **删除**：色块右上角悬停显示✕按钮，或选中后按Delete键，删除后不进合成
- **缩放**：时间轴缩放按钮，缩放后自动定位到选中片段
- **播放头**：点击时间轴标尺定位播放位置，预览视频按剪辑后的序列播放
...
| ← / → | 微调选中片段入点出点（0.5秒，Shift=0.1秒） |
| Ctrl+Z | 撤销 |
| Ctrl+Y / Ctrl+Shift+Z | 重做 |

### B-roll自动插入
- 每两段解说词之间有「➕ 插入B-roll」按钮
- 点击后AI根据上下文推荐3个候选画面片段，一键插入
- B-roll段为无配音纯画面，时间轴用虚线灰色块标记
- 支持手动输入时间范围插入，可调整/预览/删除

### 单段配音重生成
- 每段配音旁有「🔄 重生成」按钮，只重生成选中段，不用全部重来
- 修改解说词后点重生成，用新文字配音

### AI候选片段推荐
- 每段有「🤖 AI推荐」按钮，根据解说词关键词在台词+画面描述中匹配
- 返回3个最佳候选片段，标注来源（台词匹配/画面匹配）和匹配词数
- 点击即应用该片段的时间范围

## 🛡️ 稳定性与恢复机制

- **崩溃恢复**：任务运行中保存活动状态，服务崩溃/重启后页面显示黄色提示条，告知中断阶段和中间产物位置（TTS/VLM 结果可断点续跑）
- **VLM 断点续跑**：场景分析进度原子写入（指纹校验），中断后重新运行自动跳过已完成帧；取消/失败不写部分缓存，重试完整
- **TTS 断点续跑**：已生成的配音片段缓存，重跑时直接复用；失败重试 + 连续失败熔断（避免几十段逐段重试假死）
- **上传后预热**：上传完成即在空闲期后台预分析 ASR+VLM（结果入缓存），点击解说时秒级命中——同视频换文案从 40 分钟降到 3~5 分钟
- **增量缓存**：场景切点/帧信号按「文件指纹+参数+版本」落盘（200 条上限自动清最旧），卡点与解说共享同一份切点缓存
- **任务队列**：多任务排队后台跑，磁盘持久化，服务重启后恢复排队状态
- **撤销重做**：时间轴所有操作（裁剪/拖动/删除/偏移）支持 50 步撤销栈
- **原子写**：历史 / 配置 / 进度 JSON 全部原子落盘（临时文件 + `os.replace`），半截写入不再覆盖数据
- **防重入**：模型下载跨槽互斥；历史写入加锁；本地推理全局互斥（ASR/LLM/VLM 同槽串行，远端 API 并行）
- **超时与取消**：ffmpeg 整体超时 + 立即终止 + 子进程/句柄回收（取消不泄漏僵尸进程）；LLM/配音阶段协作式取消秒级感知
- **上传断点续传**：分片会话 id 存 localStorage，刷新 / 换标签页 / 服务重启均可续传；废弃会话 24h 自动清理
- **磁盘自动回收**：run_dir / 预热目录 / 上传残片纳入 6h 周期清扫（孤儿整删、历史引用只删源片副本、保成片），全程静默容错不误删

## 🧪 工程化与质量

- **模块化拆分**：后端从单文件（1.1 万行）按域拆为 `handler / workflows / movie_narrator / video_render / beat_analysis / ffmpeg_utils / cache_utils / ai_providers / tts_engines / text_utils / bili_downloader`；HTTP 端点路由表化（新增端点 = 一个方法 + 登记一行）
- **回归保障**：全量 **308 个测试 / 0 失败**（覆盖三大创作链路、并发双任务、分片上传、命令注入面、路径安全、断点续跑、磁盘清理等）；`pyflakes webui_server.py tests/` 零告警门禁；GitHub Actions CI（Python 3.9 / 3.11 双矩阵）
- **安全**：路径穿越防护（素材/音乐/媒体目录归属校验）、全项目禁 `shell=True`（模型拉取白名单校验）、前端 XSS 转义、`ai_config.json` 密钥不入库（`.gitignore` 排除）
- **质量升级（渲染）**：单遍合并编码（剪切+微变速+烧字幕+缩放一次成型）、画质三档（preset+crf / vbr+cq）、BGM 侧链闪避、有限次循环、段落边界 200ms 交叉淡化
...
- **关键词（本地/云端两条路径都支持）**：
  - 本地路径：`ai_config.json` 配 `vlm`（如 ollama `qwen3-vl:8b`），0 元跑剧情解说；
  - 云端智能：配 `chat` + `vision`（如 DeepSeek），解说质量更高（更长、更会点出作品背景）。未配 `chat` 时自动走本地路径。

> ⚠️ 本地视觉模型（如 `qwen3-vl:8b`）能输出连贯剧情旁白，但对"这是哪部作品、深层剧情冲突"的识别有限；想要《赌圣》示例那种"点片名、讲人物背景、有年代感"的解说，建议配置云端 `chat`（DeepSeek 按量计费，几毛钱）或换用更大本地模型。

### 🧠 本地部署"标准解说模型"（推荐，免费、隐私、效果接近云端）

解说靠**两个模型分工**，别指望一个视觉模型全包：

| 职责 | 模型 | 说明 |
| --- | --- | --- |
| 👁 **看懂画面**（人物/场景/事件/剧情理解） | `qwen3-vl:8b` ⭐（视觉模型，兼容 `qwen2.5vl`） | 只负责"看图"，不擅长写词 |
| 📝 **写剧情解说稿**（主力，决定解说质量） | `qwen3:14b`（文字模型，兼容 `qwen2.5:14b`） | 点片名、讲人物背景、有年代感，靠它 |

**部署（二选一）**：

```bash
# ① 网页一键拉取（推荐）：界面「🤖 AI 配置 → ③ 本地模型」点卡片下载，自动走加速通道
# ② 命令行手动拉取
ollama pull qwen3:14b          # 写稿主力（配置一般可换 qwen3:8b 或 qwen2.5:7b）
ollama pull qwen3-vl:8b        # 看图（仅当还没有视觉模型）
```

> ⚡ **国内拉取优先走"加速通道"**（网页内点「📥 拉取模型」会自动优先执行，无需手动）：
> 官方 `ollama pull` 走境外 registry 源，国内慢/卡。本项目内置白名单加速源——从 **hf-mirror** 拉官方 **单文件 GGUF** + `aria2c` 多线程下载（几 GB 约 2~3 分钟）+ `ollama create` 本地导入，网页拉取自动使用；加速通道失败时自动回退官方源。已覆盖：
> - `qwen3:14b-q4_K_M`（bartowski 单文件：`Qwen_Qwen3-14B-Q4_K_M.gguf`）
> - `qwen3-vl:8b`（官方双文件：主模型 `Qwen3VL-8B-Instruct-Q4_K_M.gguf` + 视觉投影 `mmproj-Qwen3VL-8B-Instruct-Q8_0.gguf`，Modelfile 双 `FROM` 导入）
> - `qwen2.5:14b` / `qwen2.5:7b` / `qwen2.5:latest`（bartowski 单文件）
>
> **手动加速导入（网页拉取也失败时）**：
> ```bash
> # ① 多线程下载单文件（免科学上网，hf-mirror 直连）
> aria2c -c -x 8 -s 8 -k 1M --max-tries=0 --retry-wait=3 --timeout=60 -o qwen3-14b.gguf ^
>   "https://hf-mirror.com/bartowski/Qwen_Qwen3-14B-GGUF/resolve/main/Qwen_Qwen3-14B-Q4_K_M.gguf"
...
然后在界面「🤖 AI 配置 → ③ 本地模型」把**写稿模型**填成 `qwen3:14b`，「⑤ VLM」保持视觉模型 `qwen3-vl:8b`。程序会自动用「视觉模型看画面 + 文字模型写解说」，无需改代码。
