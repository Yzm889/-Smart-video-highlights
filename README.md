...
- **崩溃恢复**：任务运行中保存活动状态，服务崩溃/重启后页面显示黄色提示条，告知中断阶段和中间产物位置（TTS/VLM结果可断点续跑）
- **VLM断点续跑**：场景分析进度原子写入，中断后重新运行自动跳过已完成帧
- **TTS断点续跑**：已生成的配音片段缓存，重跑时直接复用
- **增量缓存**：同一视频换文案时，ASR台词+VLM画面分析结果复用，从40分钟降到3-5分钟
- **任务队列**：多任务排队后台跑，磁盘持久化，服务重启后恢复排队状态
- **撤销重做**：时间轴所有操作（裁剪/拖动/删除/偏移）支持50步撤销栈
