...
> **当前状态：全部完成、132 测试通过、可正常出片。解说已切到本地 14B 模型。GPU 硬编已启用。存储管理面板已上线。🎭 剧情驱动解说已上线。第三十二轮：解说主线剪辑+风格一致。第三十三轮：配音时长自适应贴合时间轴（字数-时长换算+atempo提速+句读截断兜底）。第三十四轮：代码审查 + 时长探测性能修复（切片循环最高 281× 提速）。第三十五轮：中文字体跨平台探测 + 不静默降级（P0 兜底）。**

---

## ✅ 第三十五轮（本轮）：中文字体跨平台探测 + 不静默降级

> 背景：全景清单核查中发现的 P0 硬伤 —— `FONT_PATH` 硬编码 Windows 路径，
> 加载失败静默降级为 `ImageFont.load_default()`（**不含中文字形且不报错**）
> → macOS/Linux/Docker 上字幕与封面标题全是豆腐块，用户无从排查。

### 改动（`webui_server.py`）
1. **跨平台探测** `_resolve_cjk_font()`（惰性 + 进程内缓存），优先级：
   环境变量 `SPRING_VIDEO_FONT` → 仓库自带 `assets/fonts/` → 平台候选字体
   （Windows 6 个 / macOS 7 个 / Linux 12 个，含 Debian trixie 的 `NotoSansCJKsc-Regular.otf`）
   → 系统字体目录扫描（限流 ≤400 文件、≤8 秒）。
2. **缺字形校验** `_font_has_cjk()`：把采样汉字位图与「必然缺失」的私用区码位
   （U+E000）位图比对，完全相同即判缺字形；对连 .notdef 都不绘制的字体（如 simsun）
   退化为「是否画出笔画」判定。
   **实测判别**：微软雅黑 / 宋体 = 含中文；Arial / Segoe UI / Times / Pillow 默认字体 = 缺字形。
3. **不再静默降级**：新增 `FontMissingError` + `font_missing_help()`（各平台安装命令 +
   两种免安装兜底）。成片路径（封面/字幕）缺字体 → **中止渲染并抛错**，不留半成品。
4. **启动自检** `font_selfcheck()`：缺字体时在启动横幅里直接告警，不等用户在成片里发现。
5. **例外仅一处**：`stamp_title()`（4 张默认示例图上的装饰性标题）缺字体时
   **宁可不加字**也不画豆腐块，且只告警不中断启动 —— 避免 Linux 上起不来服务。

### 验证
- pyflakes 0 告警；**pytest 132 passed**（126 → +6 新增字体用例）。
- 三场景冒烟：① 本机解析到 `msyh.ttc`，渲染汉字墨迹 5191 px（豆腐块仅几百）；
  ② 模拟缺字体 → 抛 `FontMissingError`，提示含 `fonts-noto-cjk` 与 `SPRING_VIDEO_FONT`；
  ③ 模拟 Linux 平台 + 关闭扫描 → 明确返回空并给出 reason，不随便挑字体充数。
- 新增用例：缺字形识别 / 抛错而非降级 / 自检告警 / 解析顺序 / 封面失败不留半成品 / 示例图不加字。

### 配套
- `README.md` 新增「🔤 中文字体」章节（探测顺序 + 三种修复方式 + 商用授权提醒）。
- 新增 `assets/fonts/README.md`：字体放置说明 + OFL 字体推荐表（换字体无需改代码）。
- 清单文件 `.workbuddy/outputs/roadmap-and-expert-prompts-2026-08-29.md`：#1 标记为
  「兜底已实施」，并同步更新 ① 法务（改为只需给选型结论）与 ④ 运维（不要重复设计探测）的提示词。

### 仍待办（等法务结论）
微软雅黑版权归方正/微软，成片商用有授权风险 → 换成 SIL OFL 的思源黑体放进
`assets/fonts/` 即可生效，无需改代码。

---

## ✅ 第三十四轮：代码审查 + 时长探测性能修复

> 用户需求：整体审查代码是否还能优化得更好。先出审查报告（未改代码），确认后实施 P0 性能修复。

### 审查结论（报告：`.workbuddy/outputs/code-review-2026-08-29.md`）
- 基线：`webui_server.py` 6632 行 / 202 顶层函数；pyflakes 0 告警；无 `shell=True`/`os.system`/`eval`；
  测试 126 例覆盖约 53/202 函数（26%）；重复块 44 组；`do_POST` 161 分支（42 个 `if path ==` 平铺）。
- P0 性能：`probe_duration` 全片解码 + 切片循环放大（O(N×片长)）。
- P1 可维护性：拆包、路由表化、错误处理样板 ×5、提示词硬编码重复 6 次。
- P2：`RUNSEQ[0] += 1` 非原子、无并发上限、前端 5 处 `innerHTML` 未转义。

### 本轮修复（P0）
1. **`probe_duration` 去掉 `-f null -`** —— 该参数会让 ffmpeg **完整解码整片**。
   改为 `ffmpeg_run(['-hide_banner', '-i', path])` 只读容器头。
   （旁证：`probe_audio_len` 本来就是正确写法，说明 `-f null -` 是遗留疏漏。）
2. **新增时长缓存** `_probe_duration_cached()`：按 `(abspath, mtime_ns, size)` 失效，
   `threading.Lock` 保护，容量 512（满了整体清空）。两个 probe 函数共用。
   切片循环对同一源文件探测 N 次的问题由 O(N) 降为 O(1)。
3. 抽出公共 `_parse_duration(err)`，顶部补 `import re`。

### 实测（60s 1080p 样本）
| 场景 | 修复前 | 修复后 | 提速 |
| --- | --- | --- | --- |
| 单次探测 | 1.75s | 0.11s | 15.7× |
| 切片循环 N=5 | 9.07s | 0.12s | 74× |
| 切片循环 N=10 | 18.34s | 0.12s | 155× |
| 切片循环 N=20 | 35.64s | 0.13s | **281×** |

正确性：新旧实现同样本返回值一致（60.0 = 60.0）；不存在文件仍返回 `None`；
**缓存失效已实测**（同路径换成 3s 视频后重新探测得 3.07s，未返回旧值）。

### 验证
pyflakes 0 告警；**pytest 126 passed**（与修复前一致，无回归）。

### 待办（已列入报告，未实施）
- P0-2 封面候选抽帧（L1534）：8 帧起 8 次 ffmpeg → 一次 `select` 抽 N 帧。
- P1 拆包与路由表化；P2 并发限流与前端转义。
