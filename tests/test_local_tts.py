...
def test_tts_local_cfg_normalizes_rate(monkeypatch):
    ...
    monkeypatch.setattr(T, 'load_ai_config', lambda: {'tts_local': {'engine': 'edge', 'rate': '10'}})
    assert S.tts_local_cfg()['rate'] == '+10%'
    monkeypatch.setattr(T, 'load_ai_config', lambda: {'tts_local': {'rate': '-20%'}})
    assert S.tts_local_cfg()['rate'] == '-20%'
    monkeypatch.setattr(T, 'load_ai_config', lambda: {})
    cfg = S.tts_local_cfg()
...
def test_local_tts_prefers_edge_then_falls_back(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(T, 'load_ai_config', lambda: {'tts_local': {'engine': 'auto'}})
    out = str(tmp_path / 'a.mp3')

    monkeypatch.setattr(T, 'edge_tts_available', lambda: True)
    monkeypatch.setattr(T, 'edge_tts_speak', _ok_edge)
    monkeypatch.setattr(T, 'sherpa_tts_available', lambda: True)
    monkeypatch.setattr(T, 'sherpa_tts_speak', lambda t, p, speed=1.0: False)
    monkeypatch.setattr(T, 'sapi_tts', _ok_sapi)
...
def test_local_tts_respects_explicit_engine(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(T, 'load_ai_config', lambda: {'tts_local': {'engine': 'sapi'}})
    monkeypatch.setattr(T, 'edge_tts_available', lambda: True)
    monkeypatch.setattr(T, 'sherpa_tts_available', lambda: True)
    monkeypatch.setattr(T, 'sapi_tts', _ok_sapi)
...
def test_edge_failure_breaks_circuit(monkeypatch, tmp_path):
    ...
    T._EDGE_STATE.update(fails=0, dead_until=0.0, reason='')
    monkeypatch.setattr(T.subprocess, 'run', lambda *a, **k: _R())
    monkeypatch.setattr(T, '_EDGE_RETRY', 1)      # 关掉重试，单独验证熔断计数
    monkeypatch.setattr(T, '_EDGE_RETRY_SLEEP', 0)
    out = str(tmp_path / 'c.mp3')
    for _ in range(T._EDGE_MAX_FAILS):
    ...
    monkeypatch.setattr(T.subprocess, 'run', lambda *a, **k: _make_ok(a[0]))
    T._EDGE_STATE.update(dead_until=0.0)
...
def test_edge_speak_retries_on_transient_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(T.subprocess, 'run', flaky)
    monkeypatch.setattr(T, '_EDGE_RETRY_SLEEP', 0)
    T._EDGE_STATE.update(fails=0, dead_until=0.0, reason='')
    out = str(tmp_path / 'r.mp3')
    assert S.edge_tts_speak('测试', out) is True, '瞬时抖动应靠重试救回来'
    assert calls['n'] >= 2
    assert T._EDGE_STATE['fails'] == 0, '重试成功不应计入熔断'
...
def test_task_locks_tts_engine(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(T, 'edge_tts_speak', lambda text, out, *a, **k: _ok_edge(text, out))
    monkeypatch.setattr(T, 'sherpa_tts_available', lambda: True)
...
def test_local_tts_label_reflects_state(monkeypatch):
    ...
    monkeypatch.setattr(T, '_tts_available', lambda: False)
    monkeypatch.setattr(T, 'load_ai_config', lambda: {'tts_local': {'engine': 'auto'}})
    monkeypatch.setattr(T, 'edge_tts_available', lambda: True)
    monkeypatch.setattr(T, 'sherpa_tts_available', lambda: False)
